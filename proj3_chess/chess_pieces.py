from board import Board
from piece_properties import Color
from abc import ABC, abstractmethod
from move import Move, Spot, MoveVector
from typing import Tuple, Iterable


class ChessPiece(ABC):
    # TODO: "But the documentation requires a better description of what the class represents" literaly no idea what to write

    PIECE_NAME = ""

    def __init__(self, color: Color) -> None:
        self.color = color
        self.direction = color.direction
        self.name = color.symbolize(self.PIECE_NAME)

    def add_to_board(self, board: Board, spot: Spot) -> None:
        self.board = board
        if self.board[spot.x][spot.y]:
            raise ValueError("spot is already taken")
        self.board.board_state[spot.x][spot.y] = self

    def is_legal_move(self, spot: Spot) -> bool:
        if not self.board.is_spot_within_range(spot):
            return False
        spot_state = self.board.board_state[spot.x][spot.y]
        return spot_state == None or spot_state.color != self.color

    def does_capture_piece(self, spot: Spot) -> bool:
        spot_state = self.board.board_state[spot.x][spot.y]
        return spot_state == None or spot_state.color == self.color

    def get_spot(self) -> Spot:
        for x in range(self.board.width):
            for y in range(self.board.height):
                if self.board.board_state[x][y] == self:
                    return Spot(x=x, y=y)

    @abstractmethod
    def get_available_moves(self) -> list[Move]:
        pass

    def get_available_moves_in_directions(
        self, move_directions: Iterable[MoveVector]
    ) -> list[Move]:  # TODO: figure out type hinting
        moves = []
        spot = self.get_spot()
        for step in move_directions:
            next_move = spot + step
            while self.is_legal_move(next_move):
                moves.append(
                    Move(
                        destination=next_move,
                        does_capture_piece=self.does_capture_piece(next_move),
                        piece_symbol=self.PIECE_NAME,
                    )
                )
                if self.does_capture_piece(next_move):
                    break
                next_move = next_move + step
        return moves


class Rook(ChessPiece):
    PIECE_NAME = "R"

    def get_available_moves(self) -> list:
        move_directions = (
            MoveVector(x=0, y=1),
            MoveVector(x=0, y=-1),
            MoveVector(x=1, y=0),
            MoveVector(x=-1, y=0),
        )
        return self.get_available_moves_in_directions(move_directions=move_directions)


class Bishop(ChessPiece):
    PIECE_NAME = "B"

    def get_available_moves(self) -> list[Move]:
        MV = MoveVector
        move_directions = (
            MV(x=1, y=1),
            MV(x=-1, y=-1),
            MV(x=1, y=-1),
            MV(x=-1, y=1),
        )  # TODO: should make this a class constant?
        return self.get_available_moves_in_directions(move_directions=move_directions)


class Queen(Rook, Bishop):
    PIECE_NAME = "Q"

    def get_available_moves(self) -> list[Move]:
        return Bishop.get_available_moves(self) + Rook.get_available_moves(self)


class King(ChessPiece):
    PIECE_NAME = "K"

    def get_available_moves(self) -> list[Move]:
        spot = self.get_spot()
        moves = []
        for x_move in (-1, 0, 1):
            for y_move in (-1, 0, 1):
                next_move = Spot(spot.x + x_move, spot.y + y_move)
                if self.is_legal_move(next_move) and next_move != self.get_spot():
                    moves.append(
                        Move(
                            self.PIECE_NAME,
                            next_move,
                            self.does_capture_piece(next_move),
                        )
                    )
        return moves


class Knight(ChessPiece):
    PIECE_NAME = "N"
    KNIGHT_MOVES = [
        MoveVector(x=1, y=2),
        MoveVector(x=1, y=-2),
        MoveVector(x=2, y=1),
        MoveVector(x=2, y=-1),
        MoveVector(x=-1, y=2),
        MoveVector(x=-1, y=-2),
        MoveVector(x=-2, y=1),
        MoveVector(x=-2, y=-1),
    ]  # TODO: should be class constant?

    def get_available_moves(self) -> list[Move]:
        spot = self.get_spot()
        moves = []
        for step in self.KNIGHT_MOVES:
            next_move = spot + step
            if self.is_legal_move(next_move):
                moves.append(
                    Move(
                        destination=next_move,
                        does_capture_piece=self.does_capture_piece(next_move),
                        piece_symbol=self.PIECE_NAME,
                    )
                )
        return moves


class Pawn(ChessPiece):
    PIECE_NAME = "P"

    def get_available_moves(self) -> list[Move]:  # TODO: clean up this mess :)
        spot = self.get_spot()
        moves = []
        # move 1 forward
        next_move = Spot(x=spot + self.color.direction)
        if self.is_legal_move(next_move) and not self.does_capture_piece(next_move):
            moves.append(Move(self.PIECE_NAME, next_move, False))
        # capture piece diagonally
        for x_move in (-1, 1):
            next_move = Spot(spot.x + x_move, spot.y + self.color.direction)
            if self.is_legal_move(next_move) and self.does_capture_piece(next_move):
                moves.append(Move(self.PIECE_NAME, next_move, True))
        # TODO: add case - move 2 forward
        return moves
